...
const Foryou = () => {
  ...
  const [currentEventIndex, setCurrentEventIndex] = useState(-2);
  ...
  useEffect(() => {
    ...
    if (
      storedEvents[currentEventIndex] == null ||
      Object.keys(storedEvents[currentEventIndex]).length === 0
    ) {
      return;
    }
  ...
  return (
    <>
      {currentEvent == null || Object.keys(currentEvent).length === 0 ? (
        <View>
          <Text>No events found!</Text>
        </View>
      ) : (
        <View
          ...
          <ScrollView style={{ width: "100%" }}>
            ...
            <View
              ...
              <Button onPress={() => addEventNotInterested()}>
                Not Interested
              </Button>
